...
import styles from './ImpactBannerSection2.module.css';

/**
 * ImpactBannerSection2 — "Project with real societal impact"
 * ----------------------------------------------------------
 * Same architecture and interaction model as ImpactBanner: text left / shape
 * right on desktop, stacked below 760px, a CSS --scale variable driving all
 * font sizes and spacing, ResizeObserver-driven Three.js sizing, stages that
 * advance automatically, by drag, or by clicking a dot.
 *
 * The shape differs: a central geodesic orb with two tilted orbit rings of
 * glowing nodes, instead of section 1's single icosahedron. The traveling
 * "electric current" walks node-to-node around whichever ring the drag starts
 * on, since the rings are the adjacency graph here.
 *
 ...
 * always resumes.
 *
 * Stages come from Strapi via HeroSection3D (variant = "impact"); the
 * DESIGN / ADAPT / IMPACT copy below is the fallback when none are entered.
 *
...
const DEFAULT_STAGES: Stage[] = [
  {
    eyebrow: 'DESIGN',
    headline: 'Turning ideas into real interventions.',
    sub: 'Strategies translate into tangible outcomes for communities.',
  },
  {
    eyebrow: 'ADAPT',
    headline: 'Built on continuous feedback.',
    sub: 'Collaboration keeps every project grounded in real needs.',
  },
  {
    eyebrow: 'IMPACT',
    headline: 'Measurable results, lasting value.',
    sub: 'Every project delivers outcomes that genuinely matter.',
  },
];

/** Orbit nodes are smaller than banner 1's icosahedron vertices. */
const NODE_SCALE = 0.32;
...
export default function ImpactBannerSection2({
  stages,
  autoAdvanceMs,
  className,
}: ImpactBannerProps) {
  ...
  useEffect(() => {
    ...
    // ---- central geodesic orb ----
    const orbRadius = 1.35;
    const orbGeo = new THREE.IcosahedronGeometry(orbRadius, 1);
    const orbEdges = new THREE.EdgesGeometry(orbGeo);
    ...
    const faces = new THREE.Mesh(orbGeo, faceMat);
    shapeGroup.add(faces);

    const lineMat = new THREE.LineBasicMaterial({ color: 0xa9d8ff, transparent: true, opacity: 0.6 });
    const wireframe = new THREE.LineSegments(orbEdges, lineMat);
    shapeGroup.add(wireframe);

    const glowMat = new THREE.LineBasicMaterial({ color: 0x4db3f0, transparent: true, opacity: 0.2 });
    const glowWire = new THREE.LineSegments(orbEdges, glowMat);
    ...
    // ---- two tilted orbit rings, each a closed loop of connected nodes ----
    ...
    function buildRing(
      ...
      const startIdx = uniqueVerts.length;
      const pts: THREE.Vector3[] = [];
      for (let i = 0; i < count; i++) {
        const t = (i / count) * Math.PI * 2;
        const v = new THREE.Vector3(Math.cos(t) * ringRadius, Math.sin(t) * ringRadius, 0);
      ...
      const ringMat = new THREE.LineBasicMaterial({ color: lineColor, transparent: true, opacity: 0.5 });
      shapeGroup.add(new THREE.LineLoop(ringGeo, ringMat));

      const ringGlowMat = new THREE.LineBasicMaterial({ color: lineColor, transparent: true, opacity: 0.18 });
      const ringGlowLine = new THREE.LineLoop(ringGeo.clone(), ringGlowMat);
      ringGlowLine.scale.setScalar(1.04);
      shapeGroup.add(ringGlowLine);
    }

    buildRing(2.0, 0.35, 0.12, 8, 0xa9d8ff);
    buildRing(2.55, -0.5, -0.25, 10, 0x4db3f0);
    ...
    const currentHaloSegs = makeCurrentLayerSegments(0.055, 0.7, 0xa9d8ff, CURRENT_SEGMENTS);
