...
export default function DashboardPageInteractiveMockup() {
  ...
  const series = active.id === 'ops'
    ? [40, 52, 48, 66, 58, 73, 81]
    : active.id === 'map'
    ? [32, 46, 63, 60, 74, 71, 84]
    : [28, 36, 44, 54, 62, 76, 89];

  const bars = active.id === 'ops'
    ? [72, 56, 84, 64]
    : active.id === 'map'
    ? [48, 78, 60, 82]
    : [80, 68, 88, 74];

  const donut = active.id === 'ops'
    ? [36, 24, 20, 20]
    : active.id === 'map'
    ? [28, 32, 18, 22]
    : [42, 18, 16, 24];
  ...
  return (
    <div className="min-h-screen bg-[#071426] text-white">
      <section className="relative overflow-hidden border-b border-white/10 bg-[#071426]">
      ...
      <section className="mx-auto max-w-7xl px-6 py-14 lg:px-10">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-cyan-300/80">What we build</p>
            <h2 className="mt-2 text-3xl font-semibold">Dashboard Capabilities</h2>
          </div>
          <div className="hidden text-sm text-white/50 md:block">Designed for clarity, speed, and live exploration</div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-8 lg:px-10">
        <div className="mb-6">
          <p className="text-sm uppercase tracking-[0.22em] text-cyan-300/80">Interactive demo</p>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6">
            <div className="text-sm uppercase tracking-[0.2em] text-cyan-300/80">Selected View</div>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {active.chips.map((chip) => (
                <span key={chip} className="rounded-full border border-cyan-300/15 bg-cyan-400/10 px-3 py-2 text-xs text-cyan-100">
                  {chip}
                </span>
              ))}
            </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-4">
              <div className="rounded-2xl bg-[#0a1b35] p-4">
                ...
                <div className="mt-2 text-3xl font-semibold text-cyan-300">
                  {active.id === 'ops' ? '82%' : active.id === 'map' ? '76%' : '91%'}
                </div>
              </div>
              <div className="rounded-2xl bg-[#0a1b35] p-4">
                <div className="text-xs text-white/45">Reporting time</div>
                <div className="mt-2 text-3xl font-semibold text-cyan-300">
                  {active.id === 'ops' ? '-42%' : active.id === 'map' ? '-35%' : '-51%'}
                </div>
              </div>
            </div>
          </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {donut.map((v, i) => (
                      <div key={i} className="rounded-2xl bg-[#07152c] p-4">
                        ...
                        <div className="mt-2 text-2xl font-semibold text-cyan-300">{v}%</div>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-14 lg:px-10">
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6">
            <p className="text-sm uppercase tracking-[0.22em] text-cyan-300/80">Why dashboards matter</p>
            <h2 className="mt-2 text-3xl font-semibold">Built for clarity and better decisions</h2>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6">
            <p className="text-sm uppercase tracking-[0.22em] text-cyan-300/80">Common use cases</p>
            <h2 className="mt-2 text-3xl font-semibold">Where these dashboards create value</h2>
            <div className="mt-6 space-y-4">
              {useCases.map((item, i) => (
                <div key={item} className="flex items-center justify-between rounded-2xl bg-[#0a1b35] px-5 py-4">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 pt-6 lg:px-10">
        <div className="rounded-[32px] border border-cyan-300/10 bg-[linear-gradient(135deg,rgba(27,139,255,0.18),rgba(10,27,53,0.95))] p-8 text-center md:p-12">
          <p className="text-sm uppercase tracking-[0.24em] text-cyan-300/80">Call to action</p>
          <h2 className="mt-3 text-3xl font-semibold md:text-4xl">See your data in action</h2>
          <p className="mx-auto mt-4 max-w-2xl text-white/70 leading-8">
            This live mockup shows how your Dashboard page could feel on the site: premium, interactive, and aligned with the rest of your Digital section.
          </p>
